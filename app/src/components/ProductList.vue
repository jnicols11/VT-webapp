  ...
  <div class="min-h-screen py-10 px-4">
    ...
    <div v-if="!isLoading && !hasError && products.length === 0" class="text-center mt-20">
      ...
      <p class="text-gray-500 mt-2">
        It seems there are no products to display right now. Please check back later.
      </p>
    </div>
...
async function fetchProducts() {
  try {
    ...
    const response = await axios.get(
      `http://localhost:8080/api/products/category/${props.category}`
    ) // Update with dynamic route
